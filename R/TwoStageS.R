...
p<-matrix(p,nrow=n,ncol=m)


for (i in 1:n){
  for (k in 1:m){

if (p1[i,k]>(alpha.L0*R1[i]/m) & p1[i,k]<=(alpha.U0*S1[i]/m)) {p[i,k]<-min(2*min(p1[i,k],p2[i,k]),max(p1[i,k],p2[i,k]))} else {p[i,k]<-2}
 }
}
...
## apply two-stage BH-TSADC procedure with Simes' combination function, no pi0hat estimate
...
for (i in 1:n){
   ...
   for (k in 1:m) {
     ...
     if (k<=S1[i]-R1[i]){
         ...
         if (p.order[i,k]<=(R1[i]*alpha.L0)/m) {FDR2[i,k]=(pi0.hat[i]*p.order[i,k]*(S1[i]*alpha.U0-R1[i]*alpha.L0)/2)/(R1[i]+k)}
         if (p.order[i,k]>(R1[i]*alpha.L0)/m & p.order[i,k]<=min((2*R1[i]*alpha.L0)/m,(S1[i]*alpha.U0)/m)) {FDR2[i,k]=m*pi0.hat[i]*(p.order[i,k]*((S1[i]*alpha.U0*0.5)/m-(R1[i]*alpha.L0)/m)+p.order[i,k]*p.order[i,k]*0.5)/(R1[i]+k)}
         if (p.order[i,k]>(S1[i]*alpha.U0)/m & p.order[i,k]<=(2*R1[i]*alpha.L0)/m) {FDR2[i,k]=(pi0.hat[i]*p.order[i,k]*(S1[i]*alpha.U0-R1[i]*alpha.L0))/(R1[i]+k)}
         if (p.order[i,k]>(2*R1[i]*alpha.L0)/m & p.order[i,k]<=(S1[i]*alpha.U0)/m) {FDR2[i,k]=m*pi0.hat[i]*(p.order[i,k]*0.5*(1+S1[i]*alpha.U0/m)-(R1[i]*alpha.L0)/m)/(R1[i]+k)}
         if (p.order[i,k]>=max((2*R1[i]*alpha.L0)/m, (S1[i]*alpha.U0)/m) & p.order[i,k]<=2*(S1[i]*alpha.U0)/m) {FDR2[i,k]=m*pi0.hat[i]*(p.order[i,k]*0.5*(1+2*S1[i]*alpha.U0/m)-0.5*p.order[i,k]*p.order[i,k]-(R1[i]*alpha.L0)/m)/(R1[i]+k)}
         if (p.order[i,k]>=(2*S1[i]*alpha.U0)/m) {FDR2[i,k]=pi0.hat[i]*(S1[i]*alpha.U0-R1[i]*alpha.L0)/(R1[i]+k)}
